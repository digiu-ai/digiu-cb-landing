...
(function($) {
    ...
    /* Newsletter Form1 */
    $("#newsletterForm1").validator().on("submit", function(event) {
    	if (event.isDefaultPrevented()) {
            // handle the invalid form...
            nformError();
    ...
    function nsubmitForm1() {
        // initiate variables with form content
        nformSuccess1();
	}

    function nformSuccess1() {
        $("#newsletterForm1")[0].reset();
        nsubmitMSG1(true, "Subscribed!");
        $("input").removeClass('notEmpty'); // resets the field label after submission
    }

    function nsubmitMSG1(valid, msg) {
        if (valid) {
            var msgClasses = "h3 text-center tada animated";
        } else {
            var msgClasses = "h3 text-center";
        }
        $("#nmsgSubmit1").removeClass().addClass(msgClasses).text(msg);
    }
